...
(function () {
  ...
  if (!token) {
    console.warn("No Senpa auth token found. Make sure you are logged in on senpa.io.");
    return;
  }
  try {
    await navigator.clipboard.writeText(token);
    console.log("Senpa auth token copied to clipboard.");
  } catch {
    console.log("Token copied failed; token is present in storage.");
  }
  return token;
})();`;
  ...
  function login(raw) {
    ...
    if (!token || token.split('.').length < 3) {
      throw new Error('Paste a valid login token');
    }
    if (!isJwtAlive(token)) {
  ...
  function injectUI() {
    ...
    fab.type = 'button';
    fab.innerHTML = '<span class="dot"></span><span class="label">Account</span>';
    ...
    panel.innerHTML =
      '<div class="card">' +
        '<h2>ACCOUNT</h2>' +
        '<p class="sub">Sign in with Discord, or paste your Senpa login token.</p>' +
        '<ol>' +
          '<li>Log in at <b>https://senpa.io/web</b></li>' +
          '<li>Use Copy helper, then paste the token below</li>' +
          '<li>Click Login</li>' +
        '</ol>' +
        '<textarea class="auth-token-input" placeholder="Paste login token" spellcheck="false"></textarea>' +
        '<div class="row">' +
    ...
    function open() {
      ...
      setStatus(getToken() ? 'Signed in. Ready to play.' : 'Paste a login token, or use Discord.', '');
      input.focus();
    }
